...
func TestGCSReadAtMost(t *testing.T) {
	buildLogArtifact := NewGCSArtifact(context.Background(), fakeGCSBucket.Object("logs/example-ci-run/403/build-log.txt"), "", "build-log.txt", 500e6)
	testCases := []struct {
		name     string
...
func TestGCSReadAll(t *testing.T) {
	...
	buildLogArtifactTooLarge := NewGCSArtifact(context.Background(), fakeGCSBucket.Object("logs/example-ci-run/403/build-log.txt"), "", "build-log.txt", 20)
	testCases := []struct {
		name        string
		artifact    viewers.Artifact
		expectedErr error
...
func TestGCSSize(t *testing.T) {
	startedArtifact := NewGCSArtifact(context.Background(), fakeGCSBucket.Object("logs/example-ci-run/403/started.json"), "", "started.json", 500e6)
	testCases := []struct {
		name     string
		artifact viewers.Artifact
		expected int64
	...
	for _, tc := range testCases {
		actual, err := tc.artifact.Size()
		if err != nil {
			t.Fatalf("%s failed getting size for artifact %s, err: %v", tc.name, tc.artifact.JobPath(), err)
		}
