...
func (fa *FakeArtifact) Size() (int64, error) {
	return int64(len([]byte(fa.content))), nil
}
...
func (fa *FakeArtifact) ReadAt(b []byte, off int64) (int, error) {
	r := strings.NewReader(fa.content)
...
func TestGCSReadLastNLines(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			actual, err := LastNLinesChunked(tc.a, tc.n, fakeGCSServerChunkSize)
			if err != nil {
