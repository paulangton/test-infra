...
func NewPodLogArtifact(jobName string, buildID string, podName string, sizeLimit int64, ja podLogJobAgent) (*PodLogArtifact, error) {
	...
	if sizeLimit < 0 {
		return &PodLogArtifact{}, errInvalidSizeLimit
	}
...
func (a *PodLogArtifact) ReadAtMost(n int64) ([]byte, error) {
	...
	for byteCount < n {
		...
		if err != nil {
			...
			return p, fmt.Errorf("error reading pod log: %v", err)
		}
		p = append(p, b)
		byteCount++
	}
...
// UseContext is unsupported for pod logs
func (a *PodLogArtifact) UseContext(ctx context.Context) error {
	return viewers.ErrContextUnsupported
}
