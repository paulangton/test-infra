...
test/e2e/e2e.go:137 BeforeSuite on Node 1 failed test/e2e/e2e.go:137
	...
	testAf = NewGCSArtifactFetcher(fakeGCSClient, fakeGCSServer.URL()+"/storage/v1", false)
	var err error
	fakeGCSJobSource, err = testAf.createJobSource(testSrc)
	if err != nil {
		logrus.WithError(err).Fatal("Spyglass test main failed creating job source")
	}
	kc := fkc{
		kube.ProwJob{
